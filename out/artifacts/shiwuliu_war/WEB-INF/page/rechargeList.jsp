...
<body style="background-color: #FFFFFF;">
	<div class="mainBox">
		...
		<form action="/recharge/list.html" method="post" name="userForm" id="userForm">
			...
			<div class="filter">
				...
				<input type="text" placeholder="支付帐号" class="inpMain" name="prepayId" id="prepayId" value="${vo.prepayId }" />
						...
						<c:when test="${not empty rechargeList}">
							<c:forEach items="${rechargeList}" var="list" varStatus="vs">
								<tr class="main_info">
									...
									<td>
										<c:choose>
											<c:when test="${list.isRefund == '0'}">未退款</c:when>
											<c:when test="${list.isRefund == '1'}">已退款</c:when>
										</c:choose>
									</td>
									<td>
										<c:choose>
		...
		<div class="search_div" style="line-height: 30px; text-align: right;">
			<span style="padding-right: 15px; font-size: 20px;">充值总额：${totalRechargee.sumMoney}</span> <span
				...
				style="font-size: 20px;"><input class="btnGray" type="button" value="查询退款总额" onclick="querry();" /></span> -->
		</div>
		<div class="clear"></div>
	...
	<script type="text/javascript">
		...
		function querry() {
			...
			$.ajax({
				type : "GET",
				url : "/recharge/querryRefund.html",
