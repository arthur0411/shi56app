...
<div class="mainBox">
	<h3>月卡支付查询</h3>
	<form action="/shopOrder/list.html" method="post" name="searchForm" id="searchForm">
		<div class="filter">
			支付方式：
			...
			支付状态：
			<select name="isPayment" id="isPayment" style="vertical-align: middle;">
				<option value="">不限</option>
				<option value="no" <c:if test="${vo.is_payment=='no'}">selected</c:if>>待支付</option>
				<option value="yes" <c:if test="${vo.is_payment=='yes'}">selected</c:if>>已支付</option>
			</select>
		</div>
		<div class="filter">
			手机号：
			...
			订单号：
			<input type="text" class="inpMain" placeholder="订单号" name="orderNummmm" id="orderNummmm"
				   value="${vo.order_nummmm }"/>
		</div>
		<div class="filter">
			支付时间：
					...
					<c:when test="${not empty orderList}">
						<c:forEach items="${orderList}" var="list" varStatus="vs">
							<tr class="main_info">
								...
								<td>
									<c:choose>
										<c:when test="${list.is_payment == 'yes'}">
								...
								<td>
									<c:choose>
										<c:when test="${list.actualPayment == null }"></c:when>
										<c:otherwise>￥ ${list.actualPayment}</c:otherwise>
									</c:choose>
								</td>
								<td>
