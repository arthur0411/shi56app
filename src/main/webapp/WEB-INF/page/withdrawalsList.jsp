...
<body style="background-color: #FFFFFF;">
	<div class="mainBox">
		...
		<form action="getAll.html" method="post" name="userForm" id="userForm">
			<div class="filter" >
				...
				种类：
				<select name="wclass" id="wclass">
					<option value="">不限</option>
					<option value="0" <c:if test="${withdrawals.wclass=='0'}">selected</c:if>>退押金</option>
					<option value="1" <c:if test="${withdrawals.wclass=='1'}">selected</c:if>>返现提取</option>
				</select>
						...
						<c:when test="${not empty withdrawalsList}">
							<c:forEach items="${withdrawalsList}" var="order" varStatus="vs">
								<tr class="main_info">
									...
									<td>${order.userId }</td>
									<td>
										<c:if test="${order.wclass=='0'}">退押金 </c:if>
										<c:if test="${order.wclass=='1'}">返现提取 </c:if>
									</td>
									...
									<td>
										...
										<c:if test="${order.isConfirm=='0'}">
											...
											<!--<a class="btnGray" href="javascript:aaa();">确认</a>-->
										</c:if>
									</td>
								</tr>
							</c:forEach>
	...
	<script type="text/javascript">
		...
		function confirmWithdrawals(id,userId)
		{
			...
			//window.open ("/withdrawals/withdrawalsPage.html?id="+id, "newwindow", "height=400, width=900, toolbar =no, menubar=no, scrollbars=no, resizable=no, location=no, status=no") //写成一行
		}
