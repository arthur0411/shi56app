...
<body style="background-color: #FFFFFF;">
		...
		<form action="/shopOrder/list.html" method="post" name="userForm" id="userForm">
			<!-- <div class="filter" >
				...
				<select name="orderStatus" id="orderStatus" style="vertical-align: middle;">
					...
					<option value="9" <c:if test="${order.orderStatus==9}">selected</c:if>>未支付</option>
				...
				<select name="orderClass" id="orderClass" style="vertical-align: middle;">
					<option value="" >不限</option>
					<option value="1" <c:if test="${order.orderClass == 1}">selected</c:if>>租用</option>
					<option value="2" <c:if test="${order.orderClass == 2}">selected</c:if>>售卖</option>
						...
						<c:when test="${not empty orderList}">
							<c:forEach items="${orderList}" var="order" varStatus="vs">
								<tr class="main_info">
									...
									<td>
											<c:choose>
										       <c:when test="${order.order_class=='1'}">
										             租用
										       </c:when>
										       <c:otherwise>
										             售卖
									...
									<!--<td>
										<c:choose>
										       <c:when test="${order.totalMoney== null}"></c:when>
										       <c:otherwise>￥${order.totalMoney }</c:otherwise>
										</c:choose>	
									</td>
									...
									<td>
										<c:if test="${order.order_status=='9'}">未支付 </c:if>
									...
									<td>
										...
										| <a href="javascript:deliveryOrder('${order.order_id }','${order.order_status}');" id="orderExpress">填写物流</a>
	...
	<script type="text/javascript">
		...
		function deliveryOrder(orderId,status) {
			if(status==1)
			{
				window.location = "/shopOrder/toUpdateExpress.html?orderId=" + orderId;
			}else
			{
				alert('只能对确认中的订单进行发货');
			}
		...
		function exportwithdrawals() {
			...
			document.location = "/shopOrder/excel.html" //?status=" + status + "&mobilePhone=" + mobilePhone ;
		}
