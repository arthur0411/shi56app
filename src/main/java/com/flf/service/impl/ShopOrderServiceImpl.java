...
public class ShopOrderServiceImpl extends BaseServiceImpl implements ShopOrderService {
	...
	public boolean updateInventoryReduce(ShopOrder sporder) {
		...
		int rows;
		if (sporder.getOrderDetailList().size() > 0) {
			ShopOrderDetails details = new ShopOrderDetails();
			for (Object obj : sporder.getOrderDetailList()) {
				Map map = (Map) obj;
				details.setCommodityId(Integer.parseInt(map.get("commodity_id").toString()));
				details.setCommodityOnenum(Integer.parseInt(map.get("commodity_onenum").toString()));
	...
	@Override
	public int queryOrderByUserId(int userId) {
		return dao.getSaleOrderByUserId(userId);
	}
...
/*
	...
	@Override
	public XSSFWorkbook exportOrders() {
		...
		List<Map<String, Object>> communityPersions = dao.exportOrders();
		...
		for (int i = 0, len = communityPersions.size(); i < len; i++) {
			...
			row.createCell(0).setCellValue(getMapValue(vo.get("order_nummmm").toString()));
			row.createCell(1).setCellValue(getMapValue(getOrderClass(vo.get("order_class").toString(), returnStatus)));
			row.createCell(2).setCellValue(getMapValue(vo.get("name")));
			row.createCell(3).setCellValue(getMapValue(vo.get("phone")));
			row.createCell(4).setCellValue(getMapValue(vo.get("address")));
			row.createCell(5).setCellValue(getMapValue(vo.get("create_time")));
			row.createCell(6).setCellValue(getMapValue(vo.get("actualPayment")));
			Object ss = vo.get("paymentWay");
			if(ss == null) {
				row.createCell(7).setCellValue("");
			}else {
				row.createCell(7).setCellValue(getMapValue(getpaymentWay(ss.toString(), returnStatus)));
			}
	...
	private String getOrderClass(String orderClass, String returnStatus) {
		switch (orderClass) {
		case "1":
			returnStatus = "试戴";
			break;
	...
	private String getpaymentWay(String paymentWay, String returnStatus) {
		switch (paymentWay) {
		case "1":
			returnStatus = "微信";
			break;
	...
	private void setDefaultXSSFRow(XSSFRow row, XSSFSheet sheet) {
		...
		row.createCell(1).setCellValue("订单类别");
		row.createCell(2).setCellValue("收货人");
		row.createCell(3).setCellValue("收货电话");
		row.createCell(4).setCellValue("收货地址");
		row.createCell(5).setCellValue("下单时间");
		row.createCell(6).setCellValue("订单实付");
