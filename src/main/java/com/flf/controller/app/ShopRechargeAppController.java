				...
				if (null != recharge.getUserId() && recharge.getUserId() > 0 && recharge.getRechargeFor() != null) {
					...
					if (recharge.getPaymentTarget() != null) {
						...
						/*if (recharge.getRechargeFor() == HajRechargeUtil.ORDER_PAYMENT) {
							boolean ff = orderByOrderNo.getIsPayment() == null || orderByOrderNo.getIsPayment().equals("no");
							insertFlag = (orderByOrderNo != null && ff);
						...
						if (insertFlag) {
							...
							shopRechargeService.add(recharge);
	 						if(orderByOrderNo != null) {
	 							orderByOrderNo.setIsPayment("no");
	 							orderByOrderNo.setPaymentWay(recharge.getRechargeType());
	 							orderByOrderNo.setActualPayment(recharge.getMoney());
	 							shopOrderService.updateBySelective(orderByOrderNo);
	 						}
						...
						jsonMap.put("error", "2");
						jsonMap.put("msg", "paymentTarget为空");
					}
				} else {
					jsonMap.put("error", "0");
					jsonMap.put("msg", "为空");
