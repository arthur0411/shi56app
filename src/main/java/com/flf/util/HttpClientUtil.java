...
public class HttpClientUtil {
	 ...
	 * @return 根据url请求返回的数据
	 */
	public static String sendPost(String url, String params, String contentType, String charset, String responseCharset) {
		PostMethod post = new PostMethod(url);
		try {
			post.setRequestEntity(new StringRequestEntity(params, contentType, charset));
			HttpClient httpClient = new HttpClient();
			System.out.println(post.getQueryString() + post.getParameters());
			int result = httpClient.executeMethod(post);
			log.debug("send post result: " + result);
			if (result == 200) {
				if (Tools.isNotEmpty(responseCharset)) {
					...
					// return new
					// String(post.getResponseBodyAsString().getBytes(responseCharset),
					// charset);
				}
