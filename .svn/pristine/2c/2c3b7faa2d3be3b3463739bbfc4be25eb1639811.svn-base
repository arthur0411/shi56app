...
public class ElasticsearchUtil {
	...
	public static int createIndex(List<?> list, String indexName, String typeName, String analyzerName)
			throws Exception {
		/*
		 * int successResult = 0; String indexId = null; Map<String, Object>
		 * objMap = null; BulkRequestBuilder bulkRequest = client.prepareBulk();
		 * 
		 * for (Object obj : list) { objMap = (Map<String, Object>) obj;
		 * successResult++; JSONObject jsonObject = JSONObject.fromObject(obj);
		 * if (null != objMap && null != objMap.get("id") && objMap.size() > 0)
		 * { indexId = objMap.get("id").toString(); }
		 * bulkRequest.add(client.prepareIndex(indexName, typeName, indexId)
		 * .setSource(jsonObject.toString())); } BulkResponse bulkResponse =
		 * bulkRequest.execute().actionGet();
		 * 
		 * if (!bulkResponse.hasFailures()) { return successResult; } else {
		 * log.error(bulkResponse.buildFailureMessage()); } return
		 * successResult;
		 */
		int successResult = 0;
		String indexId = null;
	 ...
	 * @date 2014-6-5 下午2:21:31
	 */
	public static int createCommoidtyIndex(List<?> list, String indexName, String typeName, String analyzerName)
			throws Exception {
		int successResult = 0;
		String indexId = null;
		Map<String, Object> objMap = null;
